package pescaria_2.pt.ua.gboard;
...
public class GBoard extends JComponent
{
    ...
    * <BR><B>requires</B>: {@code !exists(gelem, toLine, toColumn, toLayer)}
    *
    * @param gelem  object
   ...
   public synchronized void move(Gelem gelem, int fromLine, int fromColumn, int fromLayer, int toLine, int toColumn, int toLayer)
   {
      ...
      assert !exists(gelem, toLine, toColumn, toLayer): "gelem already exists in destination";
    ...
    * <BR><B>requires</B>: {@code !exists(gelem, toLine, toColumn)}
    *
    * @param gelem  object
    * @param fromLine  the line number origin
    * @param fromColumn  the column number origin
   ...
   //@ requires !exists(gelem, toLine, toColumn);
   public synchronized void move(Gelem gelem, int fromLine, int fromColumn, int toLine, int toColumn)
   {
      ...
      assert !exists(gelem, toLine, toColumn): "gelem already exists in destination";
   ...
   public synchronized void draw(Gelem gelem, int line, int column, int layer)
   {
      ...
      // Needs to be optimized (this is a result of gelem regestering elimination)!
      ...
      updateBoard(line, column, line+gelem.numberOfLines()-1, column+gelem.numberOfColumns()-1, layer+1);
   ...
   public synchronized void erase(Gelem gelem, int line, int column, int layer)
   {
      ...
      updateBoard(line, column, line+gelem.numberOfLines()-1, column+gelem.numberOfColumns()-1, 0);
   ...
   public synchronized void erase(int line, int column, int minLayer, int maxLayer)
   {
      ...
      updateBoard(line, column, line+nLine-1, column+nColumn-1, 0);
   }
...
*/
   ...
   protected void updateBoard(int startLine, int startColumn, int endLine, int endColumn, int startLayer)
   {
            ...
            for(int cl = backwardColumn; cl <= endColumn; cl++)
            {
               if (ln >= startLine && ln <= endLine && cl >= startColumn && cl <= endColumn)
               {
                  for(int i = 0; i < boardMatrix[ly][ln][cl].size(); i++)
                  {
                     ...
                     updatesIn(allocDrawGEventTuple(boardMatrix[ly][ln][cl].get(i), ln, cl));
                  }
               }
               else
               {
                  for(int i = 0; i < boardMatrix[ly][ln][cl].size(); i++)
                  {
                     ...
                     if (ln+other.numberOfLines()-1 >= startLine &&
   ...
   protected int maxGelemNumberOfLines = 1; // Used to optimize updateBoard!
   protected int maxGelemNumberOfColumns = 1;
